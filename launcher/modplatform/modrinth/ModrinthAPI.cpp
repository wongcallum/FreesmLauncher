...
std::pair<Task::Ptr, QByteArray*> ModrinthAPI::currentVersion(const QString& hash, const QString& hash_format) const
{
    ...
    auto [action, response] =
        Net::ApiDownload::makeByteArray(QString(m_base_url + "/version_file/%1?algorithm=%2").arg(hash, hash_format));
...
std::pair<Task::Ptr, QByteArray*> ModrinthAPI::currentVersions(const QStringList& hashes, QString hash_format) const
{
    ...
    auto [action, response] = Net::ApiUpload::makeByteArray(QString(m_base_url + "/version_files"), body_raw);
    netJob->addNetAction(action);
                                                             ...
                                                             std::optional<ModPlatform::ModLoaderTypes> loaders) const
{
    ...
    auto [action, response] = Net::ApiUpload::makeByteArray(
        QString(m_base_url + "/version_file/%1/update?algorithm=%2").arg(hash, hash_format), body_raw);
    netJob->addNetAction(action);
                                                              ...
                                                              std::optional<ModPlatform::ModLoaderTypes> loaders) const
{
    ...
    QJsonDocument body(body_obj);
    auto body_raw = body.toJson();
    auto [action, response] = Net::ApiUpload::makeByteArray(QString(m_base_url + "/version_files/update"), body_raw);
    netJob->addNetAction(action);
...
std::pair<Task::Ptr, QByteArray*> ModrinthAPI::getModCategories(const QString& baseUrl)
{
    auto netJob = makeShared<NetJob>(QString("Modrinth::GetCategories"), APPLICATION->network());
    auto [action, response] = Net::ApiDownload::makeByteArray(QUrl(baseUrl + "/tag/category"));
    netJob->addNetAction(action);
